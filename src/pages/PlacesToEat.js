...
export default function PlacesToEat() {
    ...
    useEffect(() => {
        async function getData() {
            const response = await fetch("https://raw.githubusercontent.com/manypossibles/designops/master/assets/data/en/categories/placesToEat.json");
    ...
    return (
        <section className="page">
            ...
            <h1 className="categoriesTitle">Places To Eat</h1>
                ...
                {places.map(place => (
                    <article className="categories-Subpage__article" key={place.Id} >
                        <img className="img" src={place.Files[0] ? place.Files[0].Uri : imagePlaceholder} alt={place.Name} 
                            onClick={() => navigate(`/places-to-eat/${place.Id}`)}
                        />
                        ...
                        <button className="readMoreButton" onClick={() => navigate(`/places-to-eat/${place.Id}`)}>Read more</button>
                    </article>
                ))}
            </section>
